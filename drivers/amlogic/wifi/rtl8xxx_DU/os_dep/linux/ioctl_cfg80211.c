...
static int rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
...

	{
		...
		if ((p2p_ie = rtw_get_p2p_ie(pnetwork->network.IEs+ie_offset, pnetwork->network.IELength-ie_offset, NULL, &p2p_ielen))) {
			DBG_871X("P2P %c "MAC_FMT"\n", (pnetwork->network.Reserved[0] == 1) ? 'B' : 'P',
